...
- **Admin** — a **global** role, above all lobbies. Can see a list of every open lobby across the app **and close any of them**, manage the player roster (view/remove accounts across all lobbies), and delete any video/post in any lobby (moderation/cleanup). Currently just the trainee's own account — no admin-management UI planned for now.

## Lobbies are temporary

A lobby only exists while it has at least one member. When the last player leaves, the lobby closes: its guesses/points/standings are wiped (they only ever belonged to that lobby), but every video anyone submitted stays saved permanently, even after the lobby is gone.
...
**Once you enter a lobby, it takes over the whole screen** — no tab bar, no way to jump to Submit Video or Home, whether you're in the waiting room or mid-round. There are exactly two ways back: leave early (which removes the lobby from your list — rejoining later means typing its code in again, and rejoining as Player rather than Organizer, since that role already transferred to someone else the moment you left), or finish the game and return to the normal app from the Final Results screen, without losing your membership in the lobby.
