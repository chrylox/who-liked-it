...
- **`posts`** — a submitted video: `submitted_by` (→ profiles, also the owner), video URL, timestamp. No `lobby_id` — it's a personal pool. Never deleted automatically (lobby closures, standings resets, etc. never touch it) — only Admin can remove one, via moderation.
...
- ✅ **Nhost project live** (see above) — organization, app, and all three GraphQL/Auth/Storage endpoints confirmed working. No schema yet.
